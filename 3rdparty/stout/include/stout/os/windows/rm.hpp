...
{
  {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      if (error == ERROR_ACCESS_DENIED) {
        LOG(WARNING) << "Waiting for file " << narrow_stringify(path)
                     << " to be deleted";
    ...
    return Error("Timed out when waiting for file " +
        narrow_stringify(path) + " to be deleted");
  }
}
...
inline Try<Nothing> rm(T&& path)
{
  ...
  const BOOL result = os::stat::isdir(longpath)
  ...
  Try<Nothing> deleted =
      ::internal::windows::wait_on_delete(narrow_stringify(longpath));
  if (deleted.isError()) {
