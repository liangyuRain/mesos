...
#ifdef __WINDOWS__
        // Because of privilege issues, `rmdir` cannot delete the directory. We
        // need to use `wclayer` to remove it.
    ...
    const string& backend)
{
  ...
  if (!os::exists(layers)) {
    ...
    if (mkdir.isError()) {
      return Failure("Failed to create layers folder: " + mkdir.error());
    }
  }
#endif // __WINDOWS__
  ...
  if (os::exists(target)) {
    return command::wclayer_remove(Path(target))
      .onAny(
          defer(self(), [=](const Future<Nothing>& future) -> Future<Nothing> {
            if (future.isFailed()) {
              return Failure(
                  "Failed to remove existing layer '" + layerId +
                  "': " + future.failure());
            } else {
    ...
    const string& source, const string& target)
{
  ...
  if (rename.isError()) {
    return Failure(
        "Failed to move layer from '" + source + "' to '" + target +
        "': " + rename.error());
  }
