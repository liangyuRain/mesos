    ...
    const http::Response& response)
{
  ...
  if (manifest->version() == spec::ManifestVersion::V2S1) {
...
Future<Nothing> DockerFetcherPluginProcess::___fetch(
  ...
  if (manifest->version() == spec::ManifestVersion::V2S2) {
    shared_ptr<spec::v2_2::ImageManifest> s2(manifest->getV2S2().get());
    for (int i = 0; i < s2->layers_size(); i++) {
      URI blob = uri::docker::blob(
          uri.path(),                         // The 'repository'.
          s2->layers(i).digest(),             // The 'digest'.
      ...
      for (int j = 0; j < s2->layers(i).urls_size(); ++j) {
        string url = s2->layers(i).urls(j);
    ...
    shared_ptr<spec::v2::ImageManifest> s1(manifest->getV2S1().get());
    for (int i = 0; i < s1->fslayers_size(); i++) {
      URI blob = uri::docker::blob(
          uri.path(),                         // The 'repository'.
          s1->fslayers(i).blobsum(),          // The 'digest'.
          uri.host(),                         // The 'registry'.
          (uri.has_fragment()                 // The 'scheme'.
            ? Option<string>(uri.fragment())
            : None()),
